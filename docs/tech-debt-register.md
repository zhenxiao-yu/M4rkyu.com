...
|----|----------|-------------|-------|--------|--------|----------|-------|--------|
| TD-001 | Test | **In progress (2026-05-30).** Vitest runner wired (`tests/unit/`, `node` env, isolated from Playwright); 43 unit tests landed covering all content Zod schemas + auth redirect-safety helpers (`sanitizeNextPath` open-redirect guard, `resolveSiteOrigin`). Remaining: audio context (`audio-player-context.tsx`) and the `actions.ts` server-action branches (need a Supabase/Resend mock harness). | `tests/unit/`, `src/content/schemas.ts`, `src/lib/auth/redirect-url.ts`, `src/lib/audio/audio-player-context.tsx` | L | High | 3.0 | 2026-05-30 | In progress |
