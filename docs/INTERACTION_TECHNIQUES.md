...
> **Current-status note:** this document includes historical `/portal`
> allocations. The current app tree has no portal route; do not restore portal
> interactions unless a new phase explicitly reintroduces that route.
...
| Performance risk | Layout cost grows with character count. Never split strings > 40 characters. Avoid on Chinese — CJK measurement is more expensive _and_ the effect reads as gimmicky in zh. |
...
- **One signature moment per viewport** — combining lerp + shader +
  text-splitting + map-range on the same screen is _always_ over-budget.
  Pick one.
