...
// notes (status = 'ready' AND publishedAt not in the future, so a future
// date in the admin editor schedules a note), newest first; admin surfaces
// read getDbNotes directly to see every status.
//
...
export const getNotesSource = cache(async (): Promise<Note[]> => {
  ...
  const now = Date.now();
  return all.filter((note) => isPublishedNote(note, now));
});
