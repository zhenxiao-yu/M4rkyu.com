...
/**
 * The site-wide BGM engine: all transport state, the A/B crossfade
 * machinery, the lazy Web Audio analyser graph, persistence, and the
 * playlist/error-recovery logic. Returns the context `value` plus the
 * three `<audio>` refs the provider wires into the DOM.
 *
 * Split out of `audio-player-context.tsx` so the provider stays a thin
 * shell. Hook call order is identical to the old inline component, so
...
export function useAudioEngine() {
  ...
  return { value, audioARef, audioBRef, preloadAudioRef };
}
